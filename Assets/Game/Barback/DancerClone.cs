...
public class DancerClone : MonoBehaviour
{
    ...
    public void Start()
    {
        transform.localPosition = _grid64Mono.GridPositionToWorldPosition(_gridPosition);
        int dancerIndex = Random.Range(0, _dancerSprites.Count);
        _spriteRenderer.sprite = GetSprite(dancerIndex);
        _danceCoroutine = StartCoroutine(dance(_moveOpts));
    }

    public void MoveTo(Vector2Int newGridPosition, Grid64Mono grid, Dancer.MoveOpts opts)
    {
    ...
    public void SetPointingSprite(Vector2Int direction, Dancer.MoveOpts moveOpts)
    {
    ...
    private IEnumerator dance(Dancer.MoveOpts moveOpts)
    {
        while (true)
        {
            SetDancingSprite();
